...
for item in human_files_short:
    human_result.append(face_detector(item))
for item in dog_files_short:
    dog_result.append(face_detector(item))
print(human_result)
print(dog_result)
...
print("human_files 的前100张图像中，能够检测到人脸的图像占比:"+ str(Counter(human_result)[True]/100))
print("dog_files 的前100张图像中，能够检测到人脸的图像占比:"+ str(Counter(dog_result)[True]/100))
...
# 
# __回答:__ 这样的要求需要视乎实际的问题需求，如果是对证件照的识别，是合理的。但如果是对开放场合的不定向照片，则不合适。我认为，图片如果没有清晰的面部特征，也需要检测的人脸，计算机是可以进行矫正运算。但必然的，其准确率和清晰面部的准确率一定不一样。没有清晰的面部特征时，可以通过一定的模拟还原，或者相似逼近的方式来解决。但效果还是视乎其模糊程度而言了。
# 
# 
...
dog_result = []
for item in human_files_short:
    human_result.append(dog_detector(item))
for item in dog_files_short:
    dog_result.append(dog_detector(item))
print("human_files 的前100张图像中，能够检测到狗狗的图像占比:"+ str(Counter(human_result)[True]/100))
print("dog_files 的前100张图像中，能够检测到狗狗的图像占比:"+ str(Counter(dog_result)[True]/100))
...
train_tensors = paths_to_tensor(train_files[:7000]).astype('float32')/255
...
# 
# __回答:__  
# 1. 卷积层是必不可少的，池化我觉得更重要的是可以简化参数。我个人比较喜欢用Dropout，因为号称可以避免过拟合。
# 2. 上图提水的步骤相当经典，每个卷积后跟着一个pool，并且维度逐次翻倍。所以效果不错。
...
# 
# __回答:__ 
# 使用了两层Dense，有一定的神经网络模型的作用，并且一次是relu，一次是softmax，有一定的容错性。加入的Dropout，避免过拟合。
# 
# 
...
### TODO: 设计你的算法
### 自由地使用所需的代码单元数吧
def test_detector(img_path):
    img=Image.open(img_path)
    plt.imshow(img)
    plt.show()
    if dog_detector(img_path):
        result = "This dog shall be a "
    else:
        result = "This human look like a "
    result+=(VGG19_predict_breed(img_path))
    return result
...
# 2. 提出至少三点改进你的模型的想法。
# 
# __回答:__ 我觉得效果还不错，比我预想的要好。
# 改进的想法：
# 1. 我个人觉得我自己目前的模型还是比较单一，结果经典但是非常简单。
# 2. 更改还可以加入更多的特征，或者在模型之前加入更多OpenCV的处理，让图形更容易被计算机提取特征。
# 3. 对于迁移学习，目前并没有尝试适当修正迁移学习的参数值，这部分估计也能起到一些优化的作用。
# 
...
## 自由地使用所需的代码单元数吧
print(test_detector('images/1.jpeg'))
print(test_detector('images/2.jpeg'))
print(test_detector('images/3.jpeg'))
print(test_detector('images/4.jpeg'))
print(test_detector('images/5.jpeg'))
print(test_detector('images/6.jpeg'))
